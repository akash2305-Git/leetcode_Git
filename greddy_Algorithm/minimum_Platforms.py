...
same for a train but we can have arrival time of one train equal to departure time of the other. At any given instance of time,
...
class minimum_platforms:
    ...
    def min_platforms(self,n,arr,dep):
        ...
        if n==0 or n==1:
            return n
        trains=[(arr[i],1) for i in range(n)] + [(dep[i],-1) for i in range(n)]
        ...
        timetable = sorted(zip(arr,dep))
        max_platforms, on_platforms = 0 , []
        for a,d in timetable:
            while on_platforms and on_platforms[0] < a:
                heappop(on_platforms)

            heappush(on_platforms,d)
            max_platforms = max(max_platforms,len(on_platforms))
...
if __name__ == "__main__":
    ...
    mp = minimum_platforms()

    ans = mp.min_platforms(n,arr,dep)
